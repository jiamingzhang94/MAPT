...
class TokenRefiner(nn.Module):
    """
    用于对原生 token (prefix) 做细微修正的小模块。
    """
    def __init__(self, d_model: int, hidden_factor=2):
        super().__init__()
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.attn_mask = attn_mask
        # 与原始版本相同，用 config 指定 prompt 长度
        self.compound_prompt_nctx = design_details.get('maple_length', 16)
        # i==0 表示第一层，这里保持和原始写法一致
    ...
    def forward(self, inputs):
        """
        inputs: [ x, compound_prompts_deeper, counter ]
            x: [L, N, D]
            compound_prompts_deeper: List[torch.Tensor], deeper prompt for each layer
            counter: int, index indicating which prompt is used at current layer
        """
        x = inputs[0]
        compound_prompts_deeper = inputs[1]
        counter = inputs[2]

        # 如果是第一层, 不需要对 prompt 做替换(原生版本如此).
        # 但“TokenRefiner” 也可在第一层就起作用(取决于你的策略).
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                # deeper prompt turned on
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        prefix_refined = prefix + self.token_refiner(prefix)
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # x -> [77, N, D] typically
                        prefix = x[:1, :, :]
                        old_prompt = x[1 + self.compound_prompt_nctx : , :, :]

                        # 如果你也想修正文本token, 可以这样:
                        # prefix_refined = prefix + self.token_refiner(prefix)
                        # 但CLS一般只有一个token, 也可以视情况决定是否修正

                        prefix_refined = prefix  # (示例保持不变)
                        textual_context = compound_prompts_deeper[counter]
                        textual_context = textual_context.expand(prefix.shape[1], -1, -1).permute(1, 0, 2).half()

                        x = torch.cat([prefix_refined, textual_context, old_prompt], dim=0)

                        counter += 1
        else:
            # 如果 first_layer = True,
            # 也可以进行 prefix_refiner, 看设计需求
            # 这里示例默认不在第一层修正
            pass
